...
exp_paths = ['../../final_results_radam/models/softtopk_temp10/']
output_path = '.'

for e, exp_path in enumerate(exp_paths):

    if model_iter == None:
        model_path = get_newest_model(exp_path)
    else:
        model_path = os.path.join(exp_path, model_iter)

    param_path = os.path.join(exp_path, model_iter, 'param.txt')

    namespace = load_namespace(param_path)

    model_name = os.path.split(model_path)[1]
    model_iter = float(model_name.replace('.pt','').replace('model_',''))

    name = str(e) + '_' + model_name.replace('.pt', '') + '_pool=' + namespace['pooling']

    if int(namespace['k']) != 1:
        name += '_k=' + namespace['k']

    if namespace['pooling'] == 'softtopk':
        name += '_temperature=' + namespace['temperature']
        name += '_window_size=' + namespace['window_size']

    dict_[name] = {}
    if not os.path.isdir((os.path.join(output_path,name))): os.mkdir(os.path.join(output_path,name))
    print()
    print('name', name)
    ...
    # Generators
    data_set = Dataset('../../data/test/', max_images=NUMBER_OF_IMAGES, kernel_lvl=KERNEL_LVL, noise_lvl=NOISE_LVL, motion_blur_boolean=MOTION_BLUR, homo_align=HOMO_ALIGN)
    ...
    for i, (X_batch, y_labels) in enumerate(data_gen):
        ...
        for j in range(params['batch_size']):
            ...
            if i < NUMBER_OF_PLOTS and j == 0:
                plt.savefig(os.path.join(output_path,name,str(i) + '.png'), bbox_inches = 'tight', pad_inches = 0)
    ...
    dict_[name]['ssim'].to_csv(os.path.join(output_path,'ssim_' + name + '.csv'))
    dict_[name]['psnr'].to_csv(os.path.join(output_path,'psnr_' + name + '.csv'))
